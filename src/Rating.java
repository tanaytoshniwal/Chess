...
public class Rating {
	...
	public static int rateAttack(){
		int rating=0;
		int tempKingPos=AlphaBeta.kingPositionA;
		
        for (int i=0;i<64;i++) {
            switch (AlphaBeta.chessBoard[i/8][i%8]) {
                case "P": AlphaBeta.kingPositionA=i;             
	                	if(!AlphaBeta.kingSafe())
	                		rating-=64;
	                    break;
                case "R": AlphaBeta.kingPositionA=i;             
		            	if(!AlphaBeta.kingSafe())
		            		rating-=500;
		                break;                    
                case "K": AlphaBeta.kingPositionA=i;             
		            	if(!AlphaBeta.kingSafe())
		            		rating-=300;
                    break;
                case "B": AlphaBeta.kingPositionA=i;             
		            	if(!AlphaBeta.kingSafe())
		            		rating-=300;
                    break;
                case "Q": AlphaBeta.kingPositionA=i;             
		            	if(!AlphaBeta.kingSafe())
		            		rating-=900;
                    break;
            }
        }
        AlphaBeta.kingPositionA=tempKingPos;
        if(!AlphaBeta.kingSafe())
        	rating-=200;
        return rating/2;
	}
	...
	public static int rateMovability(int movesAvailableCount,int depth){
		...
		if(movesAvailableCount==0){	//checkmate or stalemate condition
			if(!AlphaBeta.kingSafe()){	//checkmate
				rating+=-200000*depth;
			}else {	//stalemate
				rating+=-150000*depth;
			}
        ...
        for (int i=0;i<64;i++) {
            switch (AlphaBeta.chessBoard[i/8][i%8]) {
                ...
                case "Q": rating+=queenBoard[i/8][i%8];
                    break;
                case "A": if (material>=1750) {rating+=kingMidBoard[i/8][i%8]; rating+=AlphaBeta.possibleA(AlphaBeta.kingPositionA).length()*10;} else
                {rating+=kingEndBoard[i/8][i%8]; rating+=AlphaBeta.possibleA(AlphaBeta.kingPositionA).length()*30;}
                    break;
            }
        }
        return rating;
